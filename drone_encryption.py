...
'''
# To run -> python drone_encryption.py --device-name Drone-1 --device-ip localhost --device-port 33500 --discovery-ip localhost --discovery-port 33333
'''
...
class Drone:
    ...
    def drone_logic(self):
        while True:
            ...
            elif not math.sqrt((self.gps[0] - self.destination[0])**2 + (self.gps[1] - self.destination[1])**2) > 1:
                if self.earthquake:
...
def discovery():
    while True:
        ...
        discovery_message = encrypt_data(encryption_key, device_name)
        try:
            ...
            while time.time() - connection_time < 5:
                try:
                    ...
                    knownDevices[decrypt_data(encryption_key, data)] = sender_address
...
def send_interest_packet(data, device):
    ...
    interest_message = "interest"+"/"+requestCode+"/"+str(device)+"/"+str(data)
    packet = encrypt_data(encryption_key, interest_message)
    ...
    if device == "none":
        ...
        if str(device)+"/"+str(data) in forwardingTable:
                device_socket.sendto(packet, forwardingTable[str(device)+"/"+str(data)])
        # If we have not seen this device before (from our forwarding table), perform flooding (contact all known devices)
            ...
            for devices in knownDevices:
                device_socket.sendto(packet, knownDevices[devices])
    else:
        device_socket.sendto(packet, knownDevices[device])
        time.sleep(.1)
...
def handle_interests(message, address):
    decrypted_message = decrypt_data(encryption_key,message)
    ...
    if requested_device == device_name:
        send_requested_data(decrypted_message, address)
    ...
    # Otherwise, forward the packet if it hasnt been already
        ...
        if str(requested_device)+"/"+str(requested_data) in forwardingTable:
            ...
            device_socket.sendto(message, forwardingTable[str(requested_device)+"/"+str(requested_data)])
...
def handle_data(message, address):
    encrypted_message = decrypt_data(encryption_key,message)
    interest_code = message.split('/')[1]
    requested_device = message.split('/')[2]
    requested_data = message.split('/')[3]
...
def send_requested_data(message, address):
    ...
    encrypted_data = encrypt_data(encryption_key,data_response)
    device_socket.sendto(encrypted_data, address)
...
def receive_messages():
    while True:
        try:
            ...
            decrypted_message = decrypt_data(encryption_key,data)
            # Check if the message is an interest request or data
            if decrypted_message.decode().split('/')[0] == "interest":
                handle_interests(data, sender_address)
            elif decrypted_message.decode.split('/')[0] == "data":
